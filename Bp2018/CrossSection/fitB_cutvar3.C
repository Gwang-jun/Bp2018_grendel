...
void fitB(int usePbPb=0, TString inputdata="" , TString inputmc="", TString trgselection="1",  TString cut="", TString cutmcgen="", int isMC=0, Double_t luminosity=1., int doweight=0, TString collsyst="PbPb", TString outputfile="", TString npfit="0", int doDataCor = 0, Float_t centmin=0., Float_t centmax=100., int varnum=0)
{
  ...
  TString cutforothers1 = "Bpt>5.0 && Bpt<7.0";
  TString cutforothers2 = "Bpt>7.0 && Bpt<10.0";
  TString cutforothers3 = "Bpt>10.0 && Bpt<15.0";
  TString cutforothers4 = "Bpt>15.0 && Bpt<20.0";
  TString cutforothers5 = "Bpt>20.0 && Bpt<30.0";
  TString cutforothers6 = "Bpt>30.0 && Bpt<50.0";
  for(int i=0; i<nvar; i++)
    {
      if(i!=varnum) cutforothers1 = Form("%s&&(%s>%f)",cutforothers1.Data(),cutvar[i].Data(),cutvarref1[i]);
      if(i!=varnum) cutforothers2 = Form("%s&&(%s>%f)",cutforothers2.Data(),cutvar[i].Data(),cutvarref2[i]);
      if(i!=varnum) cutforothers3 = Form("%s&&(%s>%f)",cutforothers3.Data(),cutvar[i].Data(),cutvarref1[i]);
      if(i!=varnum) cutforothers4 = Form("%s&&(%s>%f)",cutforothers4.Data(),cutvar[i].Data(),cutvarref1[i]);
      if(i!=varnum) cutforothers5 = Form("%s&&(%s>%f)",cutforothers5.Data(),cutvar[i].Data(),cutvarref1[i]);
      if(i!=varnum) cutforothers6 = Form("%s&&(%s>%f)",cutforothers6.Data(),cutvar[i].Data(),cutvarref1[i]);
    }

  TString cutref = Form("%s",cut.Data());
  cut = Form("%s&&(%s||%s||%s||%s||%s||%s)",cut.Data(),cutforothers1.Data(),cutforothers2.Data(),cutforothers3.Data(),cutforothers4.Data(),cutforothers5.Data(),cutforothers6.Data());

  cutforothers1 = Form("%s&&(%s>%f)",cutforothers1.Data(),cutvar[varnum].Data(),cutvarref1[varnum]);
  cutforothers2 = Form("%s&&(%s>%f)",cutforothers2.Data(),cutvar[varnum].Data(),cutvarref2[varnum]);
  cutforothers3 = Form("%s&&(%s>%f)",cutforothers3.Data(),cutvar[varnum].Data(),cutvarref3[varnum]);
  cutforothers4 = Form("%s&&(%s>%f)",cutforothers4.Data(),cutvar[varnum].Data(),cutvarref4[varnum]);
  cutforothers5 = Form("%s&&(%s>%f)",cutforothers5.Data(),cutvar[varnum].Data(),cutvarref5[varnum]);
  cutforothers6 = Form("%s&&(%s>%f)",cutforothers6.Data(),cutvar[varnum].Data(),cutvarref6[varnum]);

  TString cutforref = Form("%s&&(%s||%s||%s||%s||%s||%s)",cutref.Data(),cutforothers1.Data(),cutforothers2.Data(),cutforothers3.Data(),cutforothers4.Data(),cutforothers5.Data(),cutforothers6.Data());
  ...
  TF1* f = new TF1("f","[0]+[1]*x",fmin,fmax);
  ...
  TF1 *fit(TTree *nt, TTree *ntMC, Double_t ptmin, Double_t ptmax, int isMC, bool isPbPb, TF1* &total, Float_t centmin, Float_t centmax, TString npfit)
{
  ...
  if(isPbPb && isMC==0) 
    {
      c->SaveAs(Form("plotCutVar/Fits/%s/data_PbPb_pt%.0f-%.0f_%s_%d-thcut.png",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
      c->SaveAs(Form("plotCutVar/Fits/%s/data_PbPb_pt%.0f-%.0f_%s_%d-thcut.pdf",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
    }
  else if(isPbPb && isMC==1) 
    {
      c->SaveAs(Form("plotCutVar/Fits/%s/mc_PbPb_pt%.0f-%.0f_%s_%d-thcut.png",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
      c->SaveAs(Form("plotCutVar/Fits/%s/mc_PbPb_pt%.0f-%.0f_%s_%d-thcut.pdf",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
    }
  else if(!isPbPb && isMC==0) 
    {
      c->SaveAs(Form("plotCutVar/Fits/%s/data_pp_pt%.0f-%.0f_%s_%d-thcut.png",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
      c->SaveAs(Form("plotCutVar/Fits/%s/data_pp_pt%.0f-%.0f_%s_%d-thcut.pdf",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
    }
  else 
    {
      c->SaveAs(Form("plotCutVar/Fits/%s/mc_pp_pt%.0f-%.0f_%s_%d-thcut.png",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
      c->SaveAs(Form("plotCutVar/Fits/%s/mc_pp_pt%.0f-%.0f_%s_%d-thcut.pdf",cutvarname[VARNUM].Data(),ptmin,ptmax,cutvarname[VARNUM].Data(),stage));
    }  
  return mass;
}
