...
{
  TString inputeffname = Form("ROOTfiles/MCstudiesPbPb2D_0.5GeV_Cent%.0f-%.0f.root",centmin,centmax);
  TString outputeffname = Form("ROOTfiles/MCstudiesPbPbAverage_0.5GeV_Bsbin_Cent%.0f-%.0f.root",centmin,centmax);
  ...
  for(int i=0;i<nBins;i++)
    {
      ...
      std::cout<<"ptbin "<<ptBins[i]<<"-"<<ptBins[i+1]<<" Nevts: "<<nevts[i]<<", Average 1/(acc*eff): "<<sum[i]<<" pm "<<sumerr[i]<<std::endl;      
    }
