...
//TString bkgerf = "0.15*0.284428*[0]*([7]/(sqrt(2*3.14159)*[2])+(1-[7])/(sqrt(2*3.14159)*[8]))*(TMath::Erf((x-[4])/[5])+1)";
//TString bkgerf = "0.3*0.284426*[0]*([7]/(sqrt(2*3.14159)*[2])+(1-[7])/(sqrt(2*3.14159)*[8])+[6]/(sqrt(2*3.14159)*[9]))*(TMath::Erf((x-[4])/[5])+1)";
...
TF1* fit(float ptmin, float ptmax, int s, int b, int widVar)
{
  ...
  if(b==0) f = new TF1(Form("f%d",count),Form("%s+%s+%s",sig[s].Data(),sig_bkg[s].Data(),bkgerf.Data()), 5.0, 6.0);
    ...
    //bkpi->SetParameter(0,0.15*0.284426*f->GetParameter(0)*(f->GetParameter(7)/(sqrt(2*3.14159)*f->GetParameter(2))+(1-f->GetParameter(7))/(sqrt(2*3.14159)*f->GetParameter(8))));
    //bkpi->SetParameter(0,0.3*0.284426*f->GetParameter(0)*(f->GetParameter(7)/(sqrt(2*3.14159)*f->GetParameter(2))+(1-f->GetParameter(7))/(sqrt(2*3.14159)*f->GetParameter(8))+f->GetParameter(6)/(sqrt(2*3.14159)*f->GetParameter(9))));
  ...
  TLatex* texPDF = new TLatex(0.50,0.40,Form("%s, %s",signame[s].Data(),bkgname[b].Data()));
  //if(widVar==0) TLatex* texPDF = new TLatex(0.50,0.40,Form("%s, %s",signame[s].Data(),bkgname[b].Data()));
  //if(widVar==1) TLatex* texPDF = new TLatex(0.50,0.40,"sig. width variation");
  texPDF->SetNDC();
  texPDF->SetTextSize(0.025);
  texPDF->SetTextFont(42);
  texPDF->Draw();
  ...
  TString texper = "%";
  tex = new TLatex(0.225,0.78,Form("Cent. %.0f-%.0f%s",centm,centM,texper.Data()));
  tex->SetNDC();
  tex->SetTextColor(1);
  tex->SetTextFont(42);
  tex->SetTextSize(0.05);
  ...
  if(isCent)
    {
      ...
      if(widVar==0)
      	c->SaveAs(Form("SystPDF/Cent_%s_%s_%s_%s_%s_%.0f_%.0f.pdf",centm,centM,collisionsystem.Data(),signame[s].Data(),bkgname[b].Data(),ptmin,ptmax));
      else
      	c->SaveAs(Form("SystPDF/Cent_%s_%s_%s_%s_%.0f_%.0f.pdf",centm,centM,collisionsystem.Data(),"widvar",ptmin,ptmax));
	}*/
