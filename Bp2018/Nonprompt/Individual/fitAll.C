...
void fitAll(){
  ...
  TF1* f = new TF1(Form("f"),"[0]*TMath::Erf((x-[1])/[2]) + [0] + [3]*TMath::Gaus(x,[5],[6])/(sqrt(2*3.14159)*[6]) + [9]+[10]*x ");
  ...
  f->SetParLimits(5,5.32,5.36);
  f->SetParLimits(6,0.005,0.04);
  f->SetParLimits(7,0.005,0.5);
  f->SetParameter(3,10);
  f->SetParameter(4,0.5);
  f->SetParameter(5,5.35);
  f->SetParameter(6,0.04);
  ...
  printf("%f*TMath::Erf((x-%f)/%f)+%f+%f*TMath::Gaus(x,%f,%f)/(sqrt(2*3.14159)*%f)+%f+%f*x \n", f->GetParameter(0), f->GetParameter(1), f->GetParameter(2), f->GetParameter(0), f->GetParameter(3), f->GetParameter(5), f->GetParameter(6), f->GetParameter(6), f->GetParameter(9), f->GetParameter(10));
  ...
  TLegend* leg = new TLegend(0.55,0.52,0.80,0.65,NULL,"brNDC");
  leg->SetBorderSize(0);
  ...
  h->Write();
  f->Write();
  if(ispp) c->SaveAs(Form("plotspp/fitNP_pp.pdf"));
  else c->SaveAs(Form("plotsPbPb/fitNP_PbPb.pdf"));
}
